      ...
      {SOCIAL_LINKS.map((link) => {
        ...
        return (
          <span key={link.key} onClick={onLinkClick}>
            <IconLink href={link.href} label={link.label} icon={icon} />
          </span>
        );
      })}
    </div>
...
function NavBarFrame({
  ...
  return (
    <nav className="mx-auto flex h-[72px] w-full max-w-[1512px] items-center justify-between gap-4 px-4 text-white md:h-[123.2548828125px] md:gap-6 md:px-[clamp(24px,7.9365vw,120px)]">
      ...
      <SocialLinks className="hidden shrink-0 items-center gap-1 text-zinc-500 dark:text-zinc-400 sm:gap-5 md:flex" />
...
export default function NewHomeNavBar({
  ...
  const handleOverlaySectionClick = (item: NewHomeNavItem) => {
    setIsMenuOpen(false);
